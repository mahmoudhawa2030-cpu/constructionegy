...
export default async function AdminVerificationsListPage() {
  const t = await getTranslations("adminVerifications.list");
  const supabase = await createClient();

  const { data: rows, error } = await supabase
    .from("profiles")
    .select("id, full_name, business_verification_status, business_verification_reviewed_at")
                  ...
                  list.map((r) => (
                    <tr key={r.id} className={adminUi.tbodyRow}>
                      <td className={adminUi.td}>
                        <Link className="font-semibold text-[var(--admin-brand)] hover:underline" href={`/admin/verifications/${r.id}`}>
                          {r.full_name?.trim() || "—"}
                        </Link>
                      ...
                      <td className={`${adminUi.tdMuted} whitespace-nowrap`}>
                        {r.business_verification_reviewed_at
                          ? new Date(r.business_verification_reviewed_at).toLocaleString()
                          : "—"}
                      </td>
                    </tr>
                  ))
